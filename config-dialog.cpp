...
OBSBasicSettings::OBSBasicSettings(CanvasDock *canvas_dock, QMainWindow *parent) : QDialog(parent), canvasDock(canvas_dock)
{
	...
	listwidgetitem = new QListWidgetItem(listWidget);
	listwidgetitem->setIcon(QIcon(QString::fromUtf8(":/dibu/media/dibu-vertical.svg")));
	...
	auto guide_link = new QLabel(QString::fromUtf8("<a href=\"https://github.com/dibuplays/dibu-vertical\">") +
	...
	guide_link = new QLabel(QString::fromUtf8("<a href=\"https://github.com/dibuplays/dibu-vertical\">") +
				QString::fromUtf8(obs_module_text("ViewGuide")) + QString::fromUtf8("</a>"));
	guide_link->setOpenExternalLinks(true);
	...
	auto hotkey = GetHotkeyByName("DibuVerticalDockStartBacktrack");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockStopBacktrack");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	otherHotkey = nullptr;
	hotkey = GetHotkeyByName("DibuVerticalDockStartStreaming");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockStopStreaming");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockStartRecording");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockStopRecording");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockPause");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockUnpause");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockChapter");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	hotkey = GetHotkeyByName("DibuVerticalDockSplit");
	if (hotkey) {
		auto id = obs_hotkey_get_id(hotkey);
		std::vector<obs_key_combination_t> combos = GetCombosForHotkey(id);
		auto hn = obs_hotkey_get_name(hotkey);
	...
	connect(tsButton, &QPushButton::clicked, [] { QDesktopServices::openUrl(QUrl("https://github.com/dibuplays/dibu-vertical")); });
	...
	connect(guideButton, &QPushButton::clicked, [] { QDesktopServices::openUrl(QUrl("https://github.com/dibuplays/dibu-vertical")); });
	...
	connect(discordButton, &QPushButton::clicked, [] { QDesktopServices::openUrl(QUrl("https://github.com/dibuplays/dibu-vertical/issues")); });
	...
	const auto version =
		new QLabel(QString::fromUtf8(obs_module_text("Version")) + " " + QString::fromUtf8(PROJECT_VERSION) + " " +
			   QString::fromUtf8(obs_module_text("MadeBy")) + " <a href=\"https://github.com/dibuplays/dibu-vertical\">Dibu Plays</a>");
...
void OBSBasicSettings::SaveSettings()
{
	...
	if (sscanf(res.toUtf8().constData(), "%dx%d", &width, &height) == 2 && width > 0 && height > 0 &&
		...
		blog(LOG_INFO, "[Dibu Vertical] resolution changed from %dx%d to %dx%d", canvasDock->canvas_width,
	...
	if (canvasDock->stream_advanced_settings != sa || canvasDock->stream_encoder != se.constData()) {
		...
		if (enc && strcmp(obs_encoder_get_name(enc), "dibu_vertical_video_encoder") == 0) {
	...
	if (canvasDock->record_advanced_settings != ra || canvasDock->record_encoder != re.constData()) {
		...
		if ((canvasDock->recordOutput && !obs_output_active(canvasDock->recordOutput)) ||
			...
			if (enc && strcmp(obs_encoder_get_name(enc), "dibu_vertical_record_video_encoder") == 0) {
				obs_encoder_release(enc);
			}
		}
	}
