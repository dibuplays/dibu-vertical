...
OBS_DECLARE_MODULE()
OBS_MODULE_AUTHOR("Dibu Plays; based on Aitum Vertical");
OBS_MODULE_USE_DEFAULT_LOCALE("dibu-vertical", "en-US")
...
static void save_canvas()
{
	...
	if (!config) {
		...
		blog(LOG_INFO, "[Dibu Vertical] New configuration file");
	}
	...
	if (obs_data_save_json_safe(config, path, "tmp", "bak")) {
		blog(LOG_INFO, "[Dibu Vertical] Saved settings");
	} else {
		blog(LOG_ERROR, "[Dibu Vertical] Failed saving settings");
	}
...
bool obs_module_load(void)
{
	if (obs_get_module("aitum-stream-suite")) {
		blog(LOG_ERROR, "[Dibu Vertical] Aitum Stream Suite detected, please uninstall it to use Vertical Canvas.");
		return false;
	}

	blog(LOG_INFO, "[Dibu Vertical] loaded version %s", PROJECT_VERSION);
	...
	proc_handler_add(ph, "void dibu_vertical_get_video(in int width, in int height, out ptr video)", get_video, nullptr);
	proc_handler_add(ph, "void dibu_vertical_get_stream_settings(in int width, in int height, out ptr outputs)",
			 get_stream_settings, nullptr);
	proc_handler_add(ph, "void dibu_vertical_set_stream_settings(in int width, in int height, in ptr outputs)",
			 set_stream_settings, nullptr);
	proc_handler_add(ph, "void dibu_vertical_get_stream_output(in int width, in int height, in string name, out ptr output)",
			 get_stream_output, nullptr);
	proc_handler_add(ph, "void dibu_vertical_start_stream_output(in int width, in int height, in string name)",
			 start_stream_output, nullptr);
	proc_handler_add(ph, "void dibu_vertical_stop_stream_output(in int width, in int height, in string name)",
			 stop_stream_output, nullptr);
	proc_handler_add(ph, "void dibu_vertical_add_chapter(in int width, in int height, in string chapter_name)", add_chapter,
			 nullptr);
	proc_handler_add(ph, "void dibu_vertical_get_scene(in int width, in int height, out string scene)", get_scene, nullptr);
	proc_handler_add(ph, "void dibu_vertical_switch_scene(in int width, in int height, in string scene)", switch_scene,
			 nullptr);
...
void obs_module_post_load(void)
{
	...
	if (!config) {
		config = obs_data_create();
		blog(LOG_WARNING, "[Dibu Vertical] No configuration file loaded");
	} else {
		blog(LOG_INFO, "[Dibu Vertical] Loaded configuration file");
	}
	...
	if (!canvas) {
		...
		blog(LOG_WARNING, "[Dibu Vertical] no canvas found in configuration");
	}
	...
	if (!vendor) {
		vendor = obs_websocket_register_vendor("dibu-vertical");
	}
	if (!vendor) {
		return;
	}
	...
	// Upstream's update endpoint is intentionally not used by this independent
	// fork. Releases are delivered through the Dibu Vertical GitHub repository.
}
	  ...
	  eventFilter(BuildEventFilter())
{
	...
	if (streamOutputs.empty() && strlen(obs_data_get_string(settings, "stream_server"))) {
		...
		ss.service = obs_service_create(whip ? "whip_custom" : "rtmp_custom", "dibu_vertical_stream_service_0", nullptr,
						nullptr);
	...
	const auto scenesName = "DibuVerticalDockScenes";
	...
	const auto sourcesName = "DibuVerticalDockSources";
	...
	const auto transitionsName = "DibuVerticalDockTransitions";
	...
	transitionAudioWrapper =
		obs_source_create_private("dibu_vertical_audio_wrapper_source", "dibu_vertical_audio_wrapper_source", nullptr);
...
static obs_source_t *CreateLabel(float pixelRatio, int i)
{
	...
	struct dstr name;
	dstr_init(&name);
	dstr_printf(&name, "Dibu Vertical Preview spacing label %d", i);
...
void CanvasDock::StartVirtualCam()
{
	...
	if (virtual_cam_mode == VIRTUAL_CAMERA_VERTICAL) {
		...
		if (!multiCanvasSource) {
			multiCanvasSource =
				obs_source_create_private("dibu_vertical_multi_canvas_source", "dibu_vertical_multi_canvas_source", nullptr);
...
void CanvasDock::StartRecord()
{
	...
	if (record_advanced_settings) {
		...
		if (!recordOutput || strcmp(obs_output_get_id(recordOutput), output_id) != 0) {
			obs_output_release(recordOutput);
			recordOutput = obs_output_create(output_id, "dibu_vertical_record", nullptr, nullptr);
		}
	} else {
		...
		if (!recordOutput || strcmp(obs_output_get_id(recordOutput), obs_output_get_id(output)) != 0) {
			obs_output_release(recordOutput);
			recordOutput = obs_output_create(obs_output_get_id(output), "dibu_vertical_record", nullptr, nullptr);
		}
...
void CanvasDock::ShowNoReplayOutputError()
{
	...
	if (astrcmpi(mode, "Advanced") == 0) {
		const char *recType = config_get_string(config, "AdvOut", "RecType");
		if (astrcmpi(recType, "FFmpeg") == 0) {
			blog(LOG_WARNING, "[Dibu Vertical] error starting backtrack: custom ffmpeg");
			if (isVisible()) {
	...
	blog(LOG_WARNING, "[Dibu Vertical] error starting backtrack: no replay buffer found");
	if (isVisible()) {
		QMessageBox::warning(this, QString::fromUtf8(obs_module_text("backtrackStartFail")),
...
void CanvasDock::StartReplayBuffer()
{
	...
	if (record_advanced_settings) {
		...
		if (!enc) {
			obs_output_release(replay_output);
			blog(LOG_WARNING, "[Dibu Vertical] error starting backtrack: no video encoder found");
			return;
		}
...
obs_encoder_t *CanvasDock::GetStreamVideoEncoder()
{
	...
	if (!video_encoder) {
		video_encoder = obs_video_encoder_create(enc_id, "dibu_vertical_video_encoder", nullptr, nullptr);
	}
...
obs_encoder_t *CanvasDock::GetRecordVideoEncoder()
{
	...
	if (!video_encoder) {
		video_encoder = obs_video_encoder_create(enc_id, "dibu_vertical_record_video_encoder", nullptr, nullptr);
	}
...
void CanvasDock::StartStreamOutput(std::vector<StreamServer>::iterator it)
{
	...
	if (it->settings && obs_data_get_bool(it->settings, "advanced") && obs_get_module("aitum-multistream")) {
		blog(LOG_INFO, "[Dibu Vertical] Start output '%s' with multistream advanced settings", it->name.c_str());
		...
		if (!venc_name || venc_name[0] == '\0') {
			...
			std::string video_encoder_name = "dibu_vertical_video_encoder_";
		...
		if (!aenc_name || aenc_name[0] == '\0') {
			...
			std::string audio_encoder_name = "dibu_vertical_audio_encoder_";
		...
		blog(LOG_INFO, "[Dibu Vertical] Start output '%s'", it->name.c_str());
		obs_output_set_video_encoder(it->output, GetStreamVideoEncoder());
		obs_output_set_audio_encoder(it->output, GetStreamAudioEncoder(), 0);
	}
	it->stopping = false;
...
void CanvasDock::CreateStreamOutput(std::vector<StreamServer>::iterator it)
{
	...
	if (!it->output || strcmp(type, obs_output_get_id(it->output)) != 0) {
		...
		std::string name = "dibu_vertical_stream";
		if (!it->name.empty()) {
			name += "_";
			name += it->name;
		}
...
obs_encoder_t *CanvasDock::GetStreamAudioEncoder()
{
	...
	if (!audio_encoder) {
		audio_encoder =
			obs_audio_encoder_create("ffmpeg_aac", "dibu_vertical_audio_encoder", audio_settings, mix_idx, nullptr);
...
void CanvasDock::StartStream()
{
	...
	if (!to_start) {
		blog(LOG_WARNING, "[Dibu Vertical] No stream output to start");
...
void CanvasDock::HandleRecordError(int code, QString last_error)
{
	if (code != OBS_OUTPUT_SUCCESS) {
		if (!last_error.isEmpty()) {
			blog(LOG_WARNING, "[Dibu Vertical] record stop error %s", last_error.toUtf8().constData());
		} else {
			blog(LOG_WARNING, "[Dibu Vertical] record stop error %i", code);
		}
	}
...
void CanvasDock::OnStreamStop(int code, QString last_error, QString stream_server, QString stream_key)
{
	...
	if (code != OBS_OUTPUT_SUCCESS) {
		if (use_last_error && !last_error.isEmpty()) {
			blog(LOG_WARNING, "[Dibu Vertical] stream stop error %s", last_error.toUtf8().constData());
		} else {
			blog(LOG_WARNING, "[Dibu Vertical] stream stop error %i", code);
		}
	}
	if (encode_error) {
...
bool CanvasDock::LoadStreamOutputs(obs_data_array_t *outputs)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		std::string service_name = "dibu_vertical_stream_service_";
...
{
	blog(LOG_INFO, "------------------------------------------------");
	blog(LOG_INFO, "[Dibu Vertical] Canvas '%s' scenes:", obs_canvas_get_name(canvas));
...
void CanvasDock::AskUpdate()
{
	...
	if (mb.clickedButton() == update) {
		QDesktopServices::openUrl(QUrl(QString::fromUtf8("https://github.com/dibuplays/dibu-vertical/releases")));
